...
def update_doctor_view(request,pk):
    ...
    doctorForm=forms.DoctorForm(data=request.FILES,instance=doctor)

    other = {'mobile':doctor.mobile,
             'address':doctor.address}
    mydict={'userForm':userForm,'doctorForm':doctorForm,'other':other}
    if request.method=='POST':
        userForm=forms.DoctorUserForm(request.POST,instance=user)
        doctorForm=forms.DoctorForm(request.POST,request.FILES,instance=doctor)
        # print('form valid: ',userForm.is_valid())
        # print('form doc valid: ',doctorForm.is_valid())
        # print('form errors: ',userForm.errors)

        if userForm.is_valid() and doctorForm.is_valid():
            # userForm.save(update_fields=['first_name','last_name'])
            # user=userForm.save()
            # user.set_password(user.password)
...
def update_patient_view(request,pk):
    ...
    other = {'mobile':patient.mobile,
             'address':patient.address,
             'symptoms':patient.symptoms}

    mydict={'userForm':userForm,'patientForm':patientForm, 'other': other}
    if request.method=='POST':
